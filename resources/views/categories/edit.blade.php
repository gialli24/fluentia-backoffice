...
['name' => $category->name, 'url' => route('categories.create')]
];
@endphp
...
<div class="fl-app-content">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 pb-4">
        <h1 class="fl-page-title">Modifica categoria</h1>
    ...
    <form method="POST" action="{{ route('categories.update', $category) }}" class="fl-auth-form mb-4"
        ...
        <x-form-field>
            ...
            <x-slot:value>{{ old('name', $category->name) }}</x-slot:value>
        </x-form-field>
        ...
        <x-form-field>
            ...
            <x-slot:value>{{ old('icon', $category->icon) }}</x-slot:value>
        </x-form-field>

        <button type="submit" class="fl-btn primary w-100 mt-4">
            Aggiorna Categoria <i class="bi bi-arrow-repeat"></i>
        </button>
    </form>
</div>
@endsection